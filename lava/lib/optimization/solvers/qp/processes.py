...
class ConstraintNeurons(AbstractProcess):
    ...
    def __init__(self, **kwargs):
        ...
        self.a_out = OutPort(shape=(shape[0], 1))
        self.thresholds = Var(shape=shape, 
...
class SolutionNeurons(AbstractProcess):
    ...
    def __init__(self, **kwargs):
        ...
        self.alpha = Var(shape=shape, 
                        init=kwargs.pop("alpha", np.ones((shape[0], 1))
                                        )
                        )
        self.beta = Var(shape=shape, 
                        init=kwargs.pop("beta", np.ones((shape[0], 1))
                                        )
                        )
        self.alpha_decay_schedule = Var(shape=(1,1), 
                                        init=kwargs.pop("alpha_decay_schedule", 
                                                        10000
                                                        )
                                        )  
        self.beta_growth_schedule = Var(shape=(1,1), 
...
class ConstraintCheck(AbstractProcess):
    ...
    def __init__(self, **kwargs):
        ...
        self.constraint_bias = Var(shape=(shape[0],1), 
                                   init=kwargs.pop("constraint_bias", 0)
                                  )
        self.a_out = OutPort(shape=(shape[0], 1))
...
class GradientDynamics(AbstractProcess):
    ...
    def __init__(self, **kwargs):
        ...
        self.grad_bias = Var(shape=(shape_hess[0], 1), 
                             init=kwargs.pop("grad_bias", 
                                              np.zeros((shape_hess[0], 1))
                                            )
                            )
        self.qp_neuron_state = Var(shape=(shape_hess[0], 1),
                                   init=kwargs.pop("qp_neurons_init", 
                                                    np.zeros((shape_hess[0], 1))
                                                  )
                                   )
        self.alpha = Var(shape=(shape_hess[0], 1), 
                        init=kwargs.pop("alpha", np.ones((shape_hess[0], 1))))
        self.beta = Var(shape=(shape_hess[0], 1), 
