...
class ReadGatePyModel(PyLoihiProcessModel):
    ...
    def run_spk(self):
        ...
        if cost[0]:
            ...
            self.cost_out.send(np.array([0]).astype(np.int32))
        elif self.solution is not None:
            timestep = - np.array([self.time_step]).astype(np.int32)
            if self.min_cost <= self.target_cost:
                self._req_pause = True
            self.cost_out.send(np.array([self.min_cost]).astype(np.int32))
            self.send_pause_request.send(timestep.astype(np.int32))
