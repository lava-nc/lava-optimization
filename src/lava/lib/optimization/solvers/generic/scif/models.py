...
class BoltzmannFixed(PyLoihiProcessModel):
    ...
    def _pass0(self):
        ...
        np.clip(self.state, a_min=-(2 ** 23), a_max=2 ** 23 - 1,
                out=self.state)
        self.lfsr_buf = lfsr.copy()
        self.spk_hist_buf = self.spk_hist.copy()
        self.spk_hist_buf &= 1
        self.is_spike = self.spk_hist_buf.copy()

        continue_nxt = np.logical_not(self.refract_buffer > 0)
        self.is_spike[continue_nxt] = 1
        self.right_side[continue_nxt] = self.state[continue_nxt].copy()

        return continue_nxt

    def _pass1(self, continue_nxt):
        continue_nxt = np.logical_and(
            continue_nxt,
            np.logical_not(self.right_side < 0))
        self.right_side[continue_nxt] += self.TEMPERATURE_TIMES_TWO[continue_nxt]
        self.right_side[continue_nxt] = np.clip(
            self.right_side[continue_nxt],
            a_min=-(2 ** 23), a_max=2 ** 23 - 1)
        self.saturate[continue_nxt] = self.right_side[continue_nxt].copy()
        self.saturate[continue_nxt] = np.multiply(self.saturate[continue_nxt],
                                                  self.lfsr_buf[continue_nxt])
        self.saturate[continue_nxt] = np.ceil(self.saturate[continue_nxt]>>24)
        saturated = self.saturate > 0
        self.is_spike[np.logical_and(saturated, continue_nxt)] = 0

        continue_nxt = np.logical_and(continue_nxt, np.logical_not(saturated))
        self.right_side[continue_nxt] = np.multiply(
            self.right_side[continue_nxt],
            self.lfsr_buf[continue_nxt])
        continue_nxt = np.logical_and(
            continue_nxt, np.logical_not(self.right_side<=self.THRESHOLD))
        self.is_spike[continue_nxt] = 0

    def _pass2(self):
        self.is_spike -= self.spk_hist_buf
        continue_nxt = np.logical_not(self.is_spike == 0)
        self.s_wta[continue_nxt] = self.is_spike[continue_nxt]

    def _pass3(self):
        continue_nxt = np.logical_not(self.spk_hist_buf != 1)
        self.s_sig[continue_nxt] = self.state[continue_nxt]

        self.refract_buffer -= 1
        self.refract_buffer = np.maximum(self.refract_buffer,
                                         0)
        continue_nxt = np.logical_not(self.is_spike == 0)
        self.refract_buffer[continue_nxt] = self.refract[continue_nxt].copy()

    def _pass4(self):
        self.spk_hist <<= 1
        self.is_spike += self.spk_hist_buf
        self.spk_hist |= self.is_spike
        self.spk_hist &= 0b11111111 # Only 8 bits

    def run_spk(self) -> None:
        self.TEMPERATURE_TIMES_TWO = 2 * self.temperature
        self.THRESHOLD = (2**16 - 1) * self.temperature

        self.spk_hist_buf = np.zeros_like(self.state)
        self.lfsr_buf = np.zeros_like(self.state)
        self.is_spike = np.zeros_like(self.state)
        self.right_side = np.zeros_like(self.state)
        self.saturate = np.zeros_like(self.state)

        self.s_sig = np.zeros_like(self.state)
        self.s_wta = np.zeros_like(self.state)

        # Receive synaptic input
        #self.a_in_data = self.a_in.recv().astype(int)

        continue_nxt = self._pass0()
        self._pass1(continue_nxt)
        self._pass2()
        self._pass3()
        self._pass4()

        # Send out spikes
        self.s_sig_out.send(self.s_sig)
        self.s_wta_out.send(self.s_wta)
