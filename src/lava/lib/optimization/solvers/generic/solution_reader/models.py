...
from lava.lib.optimization.solvers.generic.dataclasses import MacroStateReader
...
class SolutionReaderModel(AbstractSubProcessModel):
	def __init__(self, proc):
		...
		self.read_gate = ReadGate(shape=var_shape, target_cost=target_cost,
								  num_in_ports=num_in_ports)
		self.solution_readout = SolutionReadout(shape=var_shape,
		...
		self.read_gate.send_pause_request.connect(self.solution_readout.timestep_in)
