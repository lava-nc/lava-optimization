...
class OptimizationSolver:
    ...
    def solve(self, config: SolverConfig = SolverConfig()) -> SolverReport:
        ...
        report: SolverReport = None
        if config.readout_mode is ReadoutMode.SINGLE_CHIP:
            report = self._run_single_chip(run_condition, run_cfg, config=config)
        return report

    def _run_single_chip(self, run_condition, run_cfg, config: SolverConfig) -> SolverReport:
        self.solver_process.run(
            condition=run_condition, 
            run_cfg=run_cfg,
            compile_config={'folded_view' : ['SolutionFinder']} \
                if config.folded_compilation else None
        )
    ...
    def _get_probed_data(self, tracker, var_name) -> ty.Optional[np.ndarray]:
        if tracker is None:
            return None
    ...
    def _get_run_config(
        ...
        if config.backend in CPUS:
            ...
            if config.readout_mode is ReadoutMode.MULTI_CHIP_TARGET:
                pdict.update({
                    ReadGate: ReadGateCModel,
                    SolutionReadout: SolutionReadoutAsyncPyModel
                })
    ...
    def _get_best_state(self, config: SolverConfig, idx: int):
        if self._is_problem_discrete():
            if config.backend in NEUROCORES:
                discrete_values = SolutionReadoutPyModel.decode_solution(
                    np.array(
                        self.solver_process.variable_assignment.get()
                            .astype(np.int8)
                    )
                )
            else:
