...
class TestCoefficientsTensors(unittest.TestCase):
    def setUp(self) -> None:
        self.coefficients_np = CoefficientTensorsMixin(np.asarray(1),
                                                       np.ones(2),
                                                       np.ones((2, 2)),
                                                       np.ones((2, 2, 2))
                                                       )
        self.coefficients_list = CoefficientTensorsMixin(1,
                                                         [1, 1],
                                                         [[1, 1],
                                                          [1, 1]],
                                                         [[[1, 1],
    ...
    def test_create_obj(self):
        for coeffs, name in ((self.coefficients_list, "Lists"),
    ...
    def test_is_ok_not_providing_intermediate_term(self):
        coefficients = CoefficientTensorsMixin(np.asarray(1),
                                               np.ones((2, 2)),
                                               np.ones((2, 2, 2))
                                               )
