...
class TestCost(unittest.TestCase):
    def setUp(self) -> None:
        self.raw_coefficients = (np.asarray(1),
                                 np.ones(2),
                                 np.ones((2, 2)),
                                 np.ones((2, 2, 2))
                                 )
        self.augmented_terms = (np.asarray(5),
                                5 * np.ones(2),
                                5 * np.ones((2, 2)),
                                5 * np.ones((2, 2, 2))
                                )
    ...
    def test_create_obj(self):
        for cost, label in [(self.cost, "not augmented"),
    ...
    def test_reset_augmented_terms(self):
        new_augmented_terms = (np.asarray(3),
                               3 * np.ones(2),
                               3 * np.ones((2, 2)),
                               3 * np.ones((2, 2, 2))
                               )
    ...
    def test_set_augmented_terms(self):
        ...
        for n, coefficient in enumerate(new_augmented_terms):
            with self.subTest(msg=f'{n}'):
                self.assertTrue((coefficient == self.cost.augmented_terms[
                    n]).all())
