...
class TestBFSNeuronProcess(unittest.TestCase):
   ...
   def test_bfs_neuron_process(self):
        ...
        # test vars 
        self.assertEqual(np.all(process.vars.counter_mem.get() == 0), 
                         True)
        self.assertEqual(np.all(process.vars.global_depth.get() == 0), 
                         True)
        self.assertEqual(np.all(process.vars.status_reg.get() == status), 
                         True)
